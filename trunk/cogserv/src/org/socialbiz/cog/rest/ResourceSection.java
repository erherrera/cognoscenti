...
import org.socialbiz.cog.SectionComments;
...
public class ResourceSection  implements NGResource
{
    ...
    public void loadSection() throws Exception
    {
        ...
        NGPageIndex ngpi = NGPageIndex.getContainerIndexByKey(lpageid);
        if (ngpi==null)
        {
    ...
    public void deleteData() throws Exception
    {
            ...
            || lname.equals("Public Attachments"))
        {
            ...
            deleteComments(ngp);
            cmsg = "Comments with id = " + lid + " is deleted";
        }else if(lname.equals("Poll")){
            deletePolls(ngp);
            cmsg = "Polls with id = " + lid + " is deleted";
        }
    ...
    private void loadDataContent(NGSection ngs, Element element_section, String dataIds) throws Exception
    {
        ...
        //TODO: eliminate this since there are no more variable sections
         if(ngs.getName().equals("Description")
            ...
            lrp.loadCommentSection(loutdoc, ngs, element_section, lar, lserverURL);
    ...
    private void deleteComments(NGPage ngp) throws Exception
    {
        NGSection ngs = ngp.getSectionOrFail(lname);
        SectionComments secCmt = (SectionComments) ngs.getFormat();
        String[] cmtList = UtilityMethods.splitOnDelimiter(lid,',');
        for(int i=0; i<cmtList.length; i++)
        {
            secCmt.removeLeaflet(cmtList[i],ngs);
        }
    }

    private void deletePolls(NGPage ngp) throws Exception
    {
        throw new ProgramLogicError("Polls are no longer supported, and deleting of them even less supported.");
    }
        ...
        AuthRequest au, String lserverURL)throws Exception
    {
        ...
        {
            makeLink(loutdoc, ngs, element_links, thisLine, lserverURL);
        }
    }
    private static void makeLink(Document loutdoc, NGSection ngs, Element element_links,
        String linkURL, String lserverURL) throws Exception
    {
        ...
        if (!isExternal)
        {
            NGPageIndex foundPI = NGPageIndex.getContainerIndexByKey(linkAddr);
        ...
        if(element_activities != null){
            for (Element element_task : DOMUtils.getChildElementsList(element_activities)) {
                ...
                if (tid == null || tid.length() == 0 || tid.equals("factory")) {
                    ...
                    gr = ngp.createGoal();
                    gr.setState(GoalRecord.STATE_STARTED);
                }
                else {
        ...
        Element secInput, AuthRequest ar) throws Exception
    {
        ...
        if(element_cmts != null){
             for (Element element_cmt : DOMUtils.getChildElementsList(element_cmts)) {
                ...
                if(note == null)
                {
                    SectionComments.addLeaflet(ar, ngs, subject, data);
                }
                else
                {
