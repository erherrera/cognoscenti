...
<%
    ...
    String reviewers   = task.getReviewers();
    ...
    //disabled for now, because this prevents normal edit capabilities when
    //you happen to be the reviewer!
    boolean reviewMode = false;
    if (uProf!=null)
    {
        reviewMode = task.isReviewer(uProf) && (state==BaseRecord.STATE_REVIEW);
    }
%>
...
<%
    if (reviewMode)
        {
%>
            <button type="submit" id="approveActBtn" name="action"
                value="Approve">Approve</button>

            <button type="submit" id="rejectActBtn" name="action"
...
%>
        ...
        <table width="98%" class="Design8">
            ...
            <tr>
                ...
                <td class="Odd"><div id="snameautocomplete" style="WIDTH: 96%;">
                    <input type="text" name="assignee" id="assignee"
                        value="<%ar.writeHtml(assignee);%>"/>
                    <div id="snamecontainer"></div>
                   </div>
                </td>
            </tr>
            <tr>
                <td>Reviewers</td>
                <td class="Odd">
                    <input type="text" name="reviewers" style="WIDTH: 96%;" value="<%ar.writeHtml(reviewers);%>"/>
                </td>
            </tr>
            <tr>
                <td>State</td>
...
%>
            ...
            <tr>
                <td>Due Date</td>
                <td class="Odd"><input type="text" name="dueDate" id="dueDate" size="20" value="<%SectionUtil.nicePrintDate(out, dueDate);%>" readonly="1" />
                    <img src="./jscalendar/img.gif" id="btn_dueDate" style="cursor: pointer;" title="Date selector"/>
                </td>
            </tr>
            <tr>
                <td>Start Date</td>
                <td class="Odd"><input type="text" name="startDate" id="startDate" size="20" value="<%SectionUtil.nicePrintDate(out, startDate);%>" readonly="1" />
                    <img src="./jscalendar/img.gif" id="btn_startDate" style="cursor: pointer;" title="Date selector"/>
                </td>
            </tr>
            <tr>
                <td>Duration (Days)</td>
...
%>
            ...
            <tr>
                ...
                <td class="Odd">
                    ...
                    onChange="calculateDuration()" readonly="1" />
                    <img src="./jscalendar/img.gif" id="btn_endDate" style="cursor: pointer;" title="Date selector"/>
                </td>
            </tr>
            <tr>
                <td>Rank</td>
...
<%
    ...
    SectionTask.plugInCalenderScript(out, "startDate", "btn_startDate");
    SectionTask.plugInCalenderScript(out, "endDate", "btn_endDate");
...
<script type="text/javascript">
    var fullnamearray = [<%=fullnameList%>];
    initautocomplete(fullnamearray);
</script>
