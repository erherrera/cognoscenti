...
<form action="<%=ar.retPath%>CommentEmail.jsp" method="get">
<td align="left">
<input type="hidden" name="p"  value="<%ar.writeHtml(p);%>">
<input type="hidden" name="oid"  value="x">
<input type="submit" name="action" value="Send Documents by Email">
</td>
</form>
...
<%
        ...
        while(e2.hasMoreElements())
        {
            ...
            String update = ar.retPath + "ReminderEdit.jsp?p="+URLEncoder.encode(ngp.getKey(), "UTF-8")
                    +"&rid="+URLEncoder.encode(rRec.getId(), "UTF-8");
            String email = ar.retPath + "ReminderEmail.jsp?p="+URLEncoder.encode(ngp.getKey(), "UTF-8")
                    +"&rid="+URLEncoder.encode(rRec.getId(), "UTF-8");
            ...
            <a href="<%ar.writeHtml(link);%>" title="view the reminder page, and optionally fulfill the reminder">
            <%ar.writeHtml(dName);%>
            </a><br/>
            ...
            <a href="<%ar.writeHtml(email);%>" title="send this reminder as email (again)">
               <img src="<%=ar.retPath%>emailIcon.gif" title="Resend this Email Reminder">
            </a>
            </td>
            <td>
    ...
    public static void displayAttachments(AuthRequest ar, NGContainer ngp)  throws Exception
    {
        ...
        for(AttachmentRecord attachment : attachmentList)
        {
            ...
            String accessLink = ar.retPath + "AccessAttachment.jsp"
                        + "?p=" + URLEncoder.encode(ngp.getKey(), "UTF-8")
                        + "&aid=" + URLEncoder.encode(id, "UTF-8");
            String emailLink = ar.retPath + "CommentEmail.jsp"
                        + "?p=" + URLEncoder.encode(ngp.getKey(), "UTF-8")
                        + "&oid=x"
                        + "&attach" + id + "=true";
            ...
            if (!ar.isStaticSite())
            {
                ...
                ar.write("<a href=\"");
                ar.writeHtml(emailLink);
                ar.write("\" title=\"Email this document to someone.\">");
                ar.write("<img src=\"");
                ar.write(ar.retPath);
                ar.write("emailIcon.gif\" title=\"Email Document\"></a>");
            }
            ar.write("</td>");
