...
%>
        <tr>
           <td><a href="WorkItem.jsp?p=<%=URLEncoder.encode(taskPage.getKey(),"UTF-8")%>&s=Tasks&id=<%=tr.getId()%>"
                title="view and modify details of the task"><img src="<%=imageName%>"></a> </td>
...
<%
        ...
        if (watchList != null)
        {
            ...
            while (e2.hasMoreElements())
            {
                ...
                NGPageIndex ngpi = NGPageIndex.getContainerIndexByKey(wr.getPageKey());
                long  changeTime = 0;
                if (ngpi==null)
                {
...
<%
    ...
    Vector adms = NGPageIndex.getAllPagesForAdmin(uProf);
