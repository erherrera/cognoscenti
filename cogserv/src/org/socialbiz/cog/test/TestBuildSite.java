...
public class TestBuildSite implements TestSet {
	...
	private void assureTestSiteNotThere() throws Exception {
		...
		NGContainer testSiteCon = NGPageIndex.getContainerByKey(siteKey);
		if (testSiteCon==null) {
			return;
		}
		
		String name = testSiteCon.getFullName();
		...
		if (!(testSiteCon instanceof NGBook)) {
			throw new Exception("Strange.  Found a site with the key '"+siteKey+"' but it is not a NGBook .... don't know what to do with it.");
		}
		
		NGBook testSite = (NGBook) testSiteCon;
		NGBook.destroySiteAndAllProjects(testSite);
	}

	
	private NGBook createNewSite() throws Exception {
		String siteName = "Test Site "+siteKey+" "+allPunct;
		NGBook testSite = NGBook.createNewSite(siteKey, siteName);
		testSite.save();
