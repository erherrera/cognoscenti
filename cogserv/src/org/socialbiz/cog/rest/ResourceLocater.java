...
public class ResourceLocater
{
    ...
    private ResourceLocater(AuthRequest ar)
    {
        lar = ar;
        lrstatus = new ResourceStatus(lar);
    }
    ...
    */
    ...
    {
        methodname = lar.req.getMethod();
        parsePath();
        lrstatus.setOpenId(lar.getBestUserId());
        lrstatus.setServerURL(serverURL);
        lrstatus.setMethod(methodname);
        lrstatus.setRequestURL(lar.req.getRequestURL().toString());

        if("PUT".equals(methodname) || "POST".equals(methodname))
        {
            DocumentBuilderFactory dfactory = DocumentBuilderFactory.newInstance();
            DocumentBuilder db = dfactory.newDocumentBuilder();
            linxml = db.parse(lar.req.getInputStream());
        }
        return getResource2();
    }
    ...
    private NGResource getResource2() throws Exception
    {
        ...
        try
        {
            ...
            if("b".equals(token0))
            {
                return ResourceBook.handleBookRequest(lar, serverURL, lrstatus, linxml);
            }
    ...
    {
        parsedPath = lar.getParsedPath();

        //TODO: not sure if the rest of this is needed
        String ctxtroot = lar.req.getContextPath();
        String requrl = lar.req.getRequestURL().toString();
    ...
    private NGResource handlePageRequest() throws Exception
    {
        ...
        ResourcePage rp = new ResourcePage(serverURL,lar);
    ...
    private NGResource handlePageSectionRequest(ResourcePage rp) throws Exception
    {
        ...
        ResourceSection rs = new ResourceSection(serverURL,lar, rp, lrstatus, linxml);
    ...
    private NGResource handleGlobalSectionRequest() throws Exception
    {
        ...
        ResourceSection rs = new ResourceSection(serverURL,lar,null, lrstatus, linxml);
    ...
    private NGResource handleUserRequest()throws Exception
    {
        ...
        ResourceUser ru = new ResourceUser(serverURL,lar);
    ...
    {
        String searchText= lar.req.getParameter("qs");
        if(searchText == null)
        {
    ...
    private NGResource handleRelayRequest() throws Exception
    {
        ...
        NGPageIndex ngpi = NGPageIndex.getContainerIndexByKey(token1);
        if (ngpi==null)
        {
        ...
        String[] nstring = {lpAddress, lrAddress};
         ...
         if("GET".equals(methodname)){
            ...
            lar.req.setAttribute("lic", freepass);
            if(subPage.startsWith("http://") || subPage.startsWith("https://")){
                ...
                String ruserid = lar.getBestUserId() + ":" + freepass;
            ...
            try {
                isEditable = SectionTask.canEditTask(ngp,lar,relayid);
            }catch(Exception e){
