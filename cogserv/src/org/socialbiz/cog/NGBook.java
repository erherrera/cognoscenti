...
/**
* An account is a collection of pages.  This allows a collection of pages
* to share a single set of members, and a particular look and feel.
* For archaic reasons called NGBook, should be NGAccount
*/
public class NGBook extends ContainerCommon implements NGContainer
{
    ...
    public NGBook(File path, Document newDoc, String nKey)
        throws Exception
    {
        super(path, newDoc);
    ...
    * old schema required members to be children of a tag 'members'
    * and also prospective memebers in a tag 'pmembers'
    * This code migrates these to the standard Role object
    * storage format, to a role called 'Executives'
    * The tag 'members' and 'pmembers' are removed from the file.
    *
    * the old format did not distinguish between executives and owners
    * so these members are migrated to both executives and owners,
    * presumably the real owner will remove the others.
    *
    * But this code, like other migration code, must be left in
    * in case there are olld book files around with the old format.
    * until 2 years after April 2011 and there are no books older than this.
    */
    private void moveOldMembersToRole()
        throws Exception
    {
    ...
    public static NGBook readBookByKey(String key)
        throws Exception
    {
        if (keyToBook==null)
        {
            //this should never happen, but if it does....
            throw new ProgramLogicError("in readBookByKey called before the account index initialzed.");
        }
        if (key==null)
        {
    ...
    private static NGBook readBookAbsolutePath(String key, File theFile)
        throws Exception
    {
        if (!theFile.exists())
        {
    ...
    {
        //might do a copy here if we fear that the receiver will corrupt this vector
        return allAccounts;
    }


    /**
    * Creates  a book with the specified name.
    * Generates the key automatically.
    */
    public static NGBook createNewBook(String name)
        throws Exception
    {
    ...
    /**
    * Creates the sepcial default book
    * This should be kept in sync with above routine.
    */
    private static NGBook createBookByKey(String key, String name)
        throws Exception
    {
    ...
    public void saveBookAs(String newKey, UserProfile user, String comment)
        throws Exception
    {
        try
        {
            ...
            throw new NGException("nugen.exception.unable.to.write.file.for.key", new Object[]{newKey}, e);
        }
    }
    ...
    */
    ...
    /**
    * The method scanAllBooks is used to initialize the indices which are
    * used to quickly find and manipulate books.  Book records are read
    * and held in memory since they are used so frequently, and there are
    ...
    * collecting, and then reinitializing all the static variables.
    * This is a "soft restart" of the nugen system, reinitializing all
    * internal data structures.  How can this be accomplished when we are
    * not sure that all other requests have been stopped?  Simply clearing
    * the data structures has the problem that some code tests and automatically
    * regenerates certain structures, and you might have the problem that
    * multiple threads might be rebuilding the index at the same time, and
    * this would be bad.
    *
    * This method was once written to create all the structures in temporary
    * variables, and then update the globals all at once, but this is
    * not entirely satisfactory because one of the methods used during
    * the initialization might use the older existing indices, which might
    * have the effect of causing a link reference from the new tables
    * to the old tables, and cause garbage collection to fail to
    * reclaim that memory.
    *
    * The cleanest method would be to clear all global variables,
    * garbage collect with nothing in memory, and then rebuild
    * completely from scratch.  That requires a special mode of the
    * server, which prevents any access during the time that the internal
    * memory is being rebuilt.  At this point in time such an admin
    * mode is not present, so there is no way to prevent access during
    * the time that the memory is being constructed.
    *
    ...
    * possibilities.  First, there could be an exception terminating
    * the processing of the request.  This is annoying to users, but
    * a necessary side effect of doing maintenance while the server is online.
    * Second, there might be an operation that causes the regeneration
    * of such data.  This is more problematic since that constructed data
    * might have a mixture of old and new links.  So this method checks
    * to see if anything has created cached values in the mean time,
    * and throws it's own self-descruct exception.
    */
    public synchronized static void scanAllBooks(String rootDirectory)
        throws Exception
    {
    ...
    public static NGBook createOrReadBookByKey(String key, String name)
    throws Exception
    {
        File theFile = NGPage.getRealPath(key+".book");
   ...
   /////////////////  Role Requests/////////////////////
    ...
    public void saveFile(AuthRequest ar, String comment)
    throws Exception
    {
        try
        {
        ...
        {
            throw new NGException("nugen.exception.unable.to.write.account.file", new Object[]{address}, e);
        }
    }
    ...
    * because older ones may have been created elsewhere, or moved,
    * but new ones created there.
    * If this has a value, then a new folder is created inside this one for the project.
    */
    ...
    /**
    * Given a new project with a key 'p', this will return the File for the
    * new project file (which does not exist yet).  There are two methods:
    *
    * 1) if a preferred location has been set, then a new folder in that
    *    will be created, and the project NGProj placed within that.
    * 2) if no preferred location, then a regular NGPage will be created
    *    in datapath folder.
    *
    * Note: p is NOT the name of the file, but the sanitized key.
    * The returned name should have the .sp suffix on it.
    */
    public File getNewProjectPath(String p) throws Exception {
        ...
        if (!NGPage.fileIsInDataPath(theFile)) {
            throw new ProgramLogicError("Somehow the NGPage file is supposed to be in the datapath, but did not turn out to be: "+theFile);
        }
