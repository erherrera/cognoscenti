...
public class LoginController extends BaseController {
    ...
    public ModelAndView changePassword(HttpServletRequest request,
        ...
        {
            throw new NGException("nugen.operation.fail.change.password.page", null , ex);
        }
        return new ModelAndView( "ChangePassword" );
    }

    @RequestMapping(value = "/{userId}/changePassword.htm")
    ...
    @RequestMapping(value = "/changePasswordAction.form")
    public void ChangePasswordAction(HttpServletRequest request,
            HttpServletResponse response) throws Exception {

        try{
            AuthRequest ar = AuthRequest.getOrCreate( request, response);
            if(!ar.isLoggedIn()){
                sendRedirectToLogin(ar, "message.loginalert.set.password",null);
                return;
            }
            ...
            if ("Save Profile".equals( option ))
            {
                ...
                {
                    throw new NGException("nugen.exception.password.should.not.ne.null",null);
                }

                if (!password.equals( password2 ))
                {
                    throw new NGException("nugen.exception.both.password.did.not.match", null);
                }
            ...
            {
                throw new ProgramLogicError( "Invalid option '" + option + "'." );
            }
            response.sendRedirect( go );
        }
        catch (Exception ex)
        {
            throw new NGException("nugen.operation.fail.change.password", null , ex);
        }
    }
    ...
    private static void manageUserProfile(HttpServletResponse response, org.socialbiz.cog.AuthRequest ar) throws Exception {
        ...
        if (option.equals("Create New Profile"))
        {
            ...
            SuperAdminLogFile.createAdminEvent(up.getKey(), ar.nowTime,up.getPreferredEmail(), AdminEvent.NEW_USER_REGISTRATION);
...
/*
    ...
    public void fujitsuProcessLeaves(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if (go==null)
            {
                //This will happen when the session has timed out while authenticating at a OpenID site.
                //User goes to site, sites there for a while, and finally logs in, it returns here and
                //we have no idea what to do with it.  Best idea is to return to the root index page.
                go = ar.baseURL;
            }
            String err = (String) session.getAttribute("login-err");
            if (err==null)
            {
                //again, coudl be lost due to session timeout.
                go = ar.baseURL;
            }
            ...
            String key = (String) session.getAttribute("user-profile-key");
            ...
            session.setAttribute("user-profile-key", null);
            ...
            if (discovered==null)
            {
                //uh-oh, this session has no record of previous attempt, what to do?
                throw new NGException("nugen.exception.authentication.fail",null);
            }

            // verify the response; ConsumerManager needs to be the same
            // (static) instance used to place the authentication request
            VerificationResult verification = manager.verify(ar.getCompleteURL(), responselist, discovered);
            if (verification==null)
            {
                throw new ProgramLogicError("manager.verify returned a null");
            }

            // examine the verification result and extract the verified identifier
            org.openid4java.discovery.Identifier verified = verification.getVerifiedId();
            if (verified == null)
            {
                String msg = "Unable to verify the supplied OpenID.  "
                +"This could be for a number of reasons.  It might be that "
                +"network connectivity to the open id provider is down, the server is off line "
                +"temporarily, or the OpenId was not entered correctly. ("+ar.getCompleteURL()+")";
            ...
            String confirmedId = verified.getIdentifier();

            UserProfile up = null;
            if (key==null)
            {

                //in this case they were not logged in, and this logs them in
                up = UserManager.findUserByAnyId(confirmedId);

                if (up==null)
                {
                    //in this case the user has proven that they are authenticated to a
                    //Particular ID, but there is no profile.  So create one.
                    up = UserManager.createUserWithId(confirmedId);
                }
                ar.setLoggedInUser(up, confirmedId,autoLogin,openid);
            }
            else
            {
                //in this case they were logged in, but just confirming
                //a second openid.
                up = UserManager.getUserProfileByKey(key);
                if (up==null)
                {
                    //this is not very likely to happen, and it should never happen in real use,
                    //so there is no reason to translate this error message.
                    throw new ProgramLogicError("somehow an attempt was made to add an openid to user ("
                        +key+") but that user profile does not exist.");
                }
                up.addId(confirmedId);
            }

            //these should be fetched only once, so deleting both of them.
            session.setAttribute("autoLogin", null);
            session.setAttribute("openid", null);

            UserManager.writeUserProfilesToFile();

            if((up.getPreferredEmail()==null)||(up.getPreferredEmail()=="")){
                response.sendRedirect(ar.baseURL + "t/inCompleteProfileAlert.htm?go="+go);
            }else{
                response.sendRedirect(go);
            }
...
*/
    ...
    public ModelAndView inCompleteProfileAlert(
        ...
        {
            AuthRequest ar = AuthRequest.getOrCreate( request, response );
            String message = ar.getMessageFromPropertyFile("message.login.to.complete.profile", null);
            ar.assertLoggedIn(message);
        } catch (Exception ex)
        {
            throw new NGException("nugen.operation.fail.complete.profile", null , ex);
        }
        return new ModelAndView("CompleteProfile");
    }
