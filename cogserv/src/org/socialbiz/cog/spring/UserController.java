...
public class UserController extends BaseController {
    ...
    public ModelAndView watchedProjects(@PathVariable String userKey,
        ...
        try{
            ...
            //if there is a source project, then set it as the hooked link
    ...
    public ModelAndView allProjects(@PathVariable String userKey,
        ...
        try{
            ...
            request.setAttribute("bookList",userBeingViewed.findAllMemberSites());
    ...
    @RequestMapping(value = "/{userKey}/ConsolidatedTasks.htm", method = RequestMethod.GET)
    public ModelAndView consolidatedTasks(@PathVariable String userKey,
        ...
        try{
            ...
            return createModelAndView(ar, userBeingViewed, "Goals", "ConsolidatedTasks");
        }catch(Exception ex){
            throw new NGException("nugen.operation.fail.usertask.page", new Object[]{userKey} , ex);
        }
    }

    @RequestMapping(value = "/{userKey}/AgentAction.form", method = RequestMethod.POST)
    public ModelAndView AgentAction(@PathVariable String userKey,
            HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        try{
            AuthRequest ar = AuthRequest.getOrCreate(request, response);
    ...
    throws Exception {
        ...
        try{
            ...
            if(!"Stop All Notifications".equals(action)){
                ...
                ngp = (NGPage) NGPageIndex.getContainerByKeyOrFail(p);
    ...
    @RequestMapping(value = "/invitedUserRegistration.htm", method = RequestMethod.GET)
    public ModelAndView invitedUserRegistrationForm(
            HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        ...
        try{
            ...
            throw new NGException("nugen.operation.fail.invited.user.registration.page", null , ex);
        }
        return modelAndView;
    }
    ...
    public void invitedUserRegitsrationSubmit(HttpServletRequest request, HttpServletResponse response) throws Exception{
        ...
        try{
            ...
            String emailId = ar.reqParam("email").trim();
            ...
            String password2 = ar.reqParam("re-password");

            if (password.length()==0 || password2.length()==0) {
                throw new NGException("nugen.exceptionhandling.require.nonempty.parameter",null);
            }
            ...
            throw new NGException("nugen.operation.fail.invited.user.registration", null , ex);
        }
    ...
    throws Exception {
        ...
        try{
            ar = AuthRequest.getOrCreate(request, response);
            ar.assertLoggedIn("Must be logged in to request to join or leave a role.");

            UserProfile uProf = ar.getUserProfile();
            String p = ar.reqParam("pageId");
            NGPage page =  (NGPage)NGPageIndex.getContainerByKeyOrFail(p);
            ar.setPageAccessLevels(page);
            String roleName = ar.reqParam("roleName").trim();
            String action = ar.reqParam("action");
            String msgType = Constant.SUCCESS;
            String errorMsg = "";
            boolean isAdminOrExecutive = false;
            String previlageMsg = "";
            RoleRequestRecord record = null;
            if ("join_role".equals(action))
            {
                String requestDescription = ar.defParam("requestDescription","");
                String requestedBy = uProf.getUniversalId();
                boolean isAlreadyRequested = page.isAlreadyRequested(roleName, requestedBy);
                if(isAlreadyRequested){
                    msgType = Constant.FAILURE;
                    errorMsg = ar.getMessageFromPropertyFile("message.request.is.already.registered", null);
                }else{
                    ...
                    boolean isAdmin = adminRole.isPlayer(thisRequester);
                    boolean isExecutive = executiveRole.isPlayer(thisRequester);
                    ...
                    if(isAdmin || (isExecutive && "Members".equals(roleName)) || noAdmin ) {
                        ...
                        if (isAdmin) {
                            ...
                            previlageMsg = ar.getMessageFromPropertyFile("nugen.project.role.immediateapproval.noadmin", new Object[]{roleName});
                        }
                        else {
                            previlageMsg = ar.getMessageFromPropertyFile("nugen.project.role.immediateapproval.exec", new Object[]{roleName});
                        }
                    }
                    else{
                        NGWebUtils.sendRoleRequestEmail(ar,record,page);
                    }
                }
            }
            else if ("leave_role".equals(action))
            {
                NGRole role = page.getRoleOrFail(roleName);
                role.removePlayer(new AddressListEntry(uProf));
            }
            else
            {
                throw new NGException("nugen.exceptionhandling.system.not.understand.action",new Object[]{action});
            ...
            page.saveContent( ar, responseMessage);

            JSONObject paramMap = new JSONObject();
            paramMap.put(Constant.MSG_TYPE , msgType);
            paramMap.put("action", action);
            paramMap.put("roleName", roleName);
            paramMap.put(Constant.MESSAGE, errorMsg);
            paramMap.put(Constant.COMMENTS, errorMsg);
            if(isAdminOrExecutive){
                paramMap.put("isAdminOrExecutive", "yes");
                paramMap.put("previlageMsg", previlageMsg);
            }else{
                paramMap.put("isAdminOrExecutive", "no");
            }
            responseMessage = paramMap.toString();
            ...
            responseMessage = NGWebUtils.getExceptionMessageForAjaxRequest(ex, ar.getLocale());
            ar.logException("Caught by requestToJoinOrLeaveRole.ajax", ex);
        }
    ...
    throws Exception {
        ...
        try{
            ...
            String p = ar.reqParam("pageId");
            NGPage project = (NGPage)NGPageIndex.getContainerByKeyOrFail(p);
            ...
            if ("approved".equals(action))
            {
                ...
                project.save(modUser, ar.nowTime, "Add New Member ("+requestedBy+") to Role "+roleName);
            ...
            else if ("rejected".equals(action))
            {
                ...
                project.save(modUser, ar.nowTime, "Rejected role request");
    ...
    @RequestMapping(value = "/approveOrRejectRoleReqThroughMail.htm", method = RequestMethod.GET)
    public ModelAndView gotoApproveOrRejectRoleReqPage(
        ...
        try{
            ...
            NGPageIndex ngpi = NGPageIndex.getContainerIndexByKey(pageId);
            request.setAttribute("book", ngpi.pageBookKey);
    ...
    */
    ...
    @RequestMapping(value="/{userKey}/contacts.htm", method = RequestMethod.GET)
    public ModelAndView contacts(
        ...
        try{
            ...
            Map<String, String> map = (Map<String, String>)request.getSession().getAttribute("contactList");

            modelAndView = new ModelAndView("Contacts");
            modelAndView.addObject("contactList", map);
            ...
            throw new NGException("nugen.operation.fail.upload.contacts.page", new Object[]{userKey}, ex);
        }
        return modelAndView;
    }

    @RequestMapping(value="/{userKey}/addUserContacts.form", method = RequestMethod.POST)
    public ModelAndView addUserContacts(
            @PathVariable String userKey,
            HttpServletRequest request,
            HttpServletResponse response) throws Exception {

        try{
            AuthRequest ar = NGWebUtils.getAuthRequest(request, response, "Can not add user contacts.");
            request.getSession().removeAttribute("contactList");

            List<String> contactList = Arrays.asList(ar.req.getParameterValues("contactList"));

            List<AddressListEntry> newContactListToAdd = new ArrayList<AddressListEntry>();
            for (String nameAndId : contactList) {
                String[] nameAndIdVal = UtilityMethods.splitOnDelimiter(nameAndId, ':');//nameAndId.split(":");
                AddressListEntry ale = null;
                if(nameAndIdVal[1] != null && nameAndIdVal[1].trim().length() > 0){
                    ale = new AddressListEntry(nameAndIdVal[1]);
                }

                if(ale != null){
                    if(ale.getUserProfile() != null){
                        ale.setNamePart(nameAndIdVal[0]);
                    }
                    newContactListToAdd.add(ale);
                }

            }
            NGWebUtils.addMembersInContacts(ar, newContactListToAdd);
            return redirectBrowser(ar,"userContacts.htm");

        }catch(Exception ex){
            throw new NGException("nugen.operation.fail.add.user.contacts", new Object[]{userKey}, ex);
        }
    }

    @RequestMapping(value="/{userKey}/updateUserContacts.form", method = RequestMethod.POST)
    public ModelAndView updateUserContacts(
            @PathVariable String userKey,
            HttpServletRequest request,
            HttpServletResponse response) throws Exception {
        ...
        try{
            ...
            String emailIds = ar.defParam("emailIds", "");  //form field might be empty by user
            if (emailIds.length()>0) {
                NGWebUtils.updateUserContactAndSaveUserPage(ar, op, emailIds);
            }

            return redirectBrowser(ar,"userContacts.htm");

        }catch(Exception ex){
            throw new NGException("nugen.operation.fail.update.user.contacts", new Object[]{userKey}, ex);
        }
    }
    ...
    public void updateContacts(
        ...
        try{
            ...
            String emailIds = ar.reqParam("emailIds");
            NGWebUtils.updateUserContactAndSaveUserPage(ar, op,emailIds);

            JSONObject parameters = new JSONObject();
            parameters.put("action", op);
            parameters.put(Constant.MSG_TYPE , Constant.SUCCESS);

            if(emailIds.trim().length()>0 && op.equals("Remove")){
                JSONArray array = getPeopleListInJSONArray(ar, "");
            ...
            responseMessage = NGWebUtils.getExceptionMessageForAjaxRequest(ex, ar.getLocale());
            ar.logException("Caught by updateContacts.ajax", ex);
        }
        NGWebUtils.sendResponse(ar, responseMessage);
    }
    ...
    @RequestMapping(value = "/createProfile.ajax", method = RequestMethod.POST)
    ...
    throws Exception {
        ...
        try{
            ...
            responseMessage = NGWebUtils.getExceptionMessageForAjaxRequest(ex, ar.getLocale());
            ar.logException("Caught by createProfile.ajax", ex);
        }
        NGWebUtils.sendResponse(ar, responseMessage);

    }

    @RequestMapping(value = "/completeProfile.form", method = RequestMethod.POST)
    public ModelAndView completeProfile(HttpServletRequest request, HttpServletResponse response)
    throws Exception {
        String popupMsg = "";
        String go = "";
        try{
            ...
            if (password==null || password.length()==0) {
                throw new NGException("nugen.exception.enter.password",null);
            }

            UserProfile up = UserManager.findUserByAnyIdOrFail(email);
            String recordedPass = up.getPassword();
            if (recordedPass==null || recordedPass.length()==0 || !recordedPass.equals(password) )
            {
                popupMsg = "err";
                go = ar.baseURL + "t/inCompleteProfileAlert.htm?popupMsg="+popupMsg;
                return redirectBrowser(ar,go);
            }else{
                up.addId(uProf.getLastLoginId());
                ar.setLoggedInUser(up, uProf.getLastLoginId(),null,null);
                popupMsg = uProf.getLastLoginId();
                uProf.setDisabled(true);
                UserManager.removeDisabledUsers();
                go = ar.defParam("go", ar.retPath+"v/"+up.getKey()+"/watchedProjects.htm?popupMsg="+popupMsg);
                String path = ar.baseURL + "t/inCompleteProfileAlert.htm?popupMsg="+URLEncoder.encode(popupMsg,"UTF-8")+"&go="+URLEncoder.encode(go,"UTF-8");
                return redirectBrowser(ar,path);
            }

        }catch(Exception ex){
            throw new NGException("nugen.operation.fail.to.complete.profile", null, ex);
        }
    }
    ...
    public ModelAndView saveNotificationSettings(
        ...
        try{
            ...
            ngp.save(up.getUniversalId(),ar.nowTime,"updated notification settings");
    ...
    throws Exception {
        ...
        for (AddressListEntry ale : list) {
            ...
            if(profile != null){
                ...
                AuthRequest clone = new AuthDummy(profile, owner);
                ale.writeLink(clone);
