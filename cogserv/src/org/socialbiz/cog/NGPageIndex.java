...
*
* There exists a collection of NGPage objects (process leaves).  This class helps
* to form an index to those objects and to provide basic metadata about those
* pages without having to have all pages (all leaves) in memory at the same time.
* The index holds the page name, page key, references from that page, pages that
* refer to that page, which book, last change, whether a request for membership is
* pending, and who the admins (authors) of the page are.  These values are selected
* in order to allow for quick searching and listing of exisitng pages according
* to certain common criteria.  Most importantly, this allows for back-links to be
* detected readily: since we have a list of forward links, we can scan through there
* and build a list of back links.
*
* Forward links are found by using a method on NGPage to find all the outbound links.
* In this case, a link is string using wiki-link rules (can be a name and link together
* separated by vertical bar).  This can also include external links, which are ignored
* by this index.  Internal links are simply the name of leaf being addressed.  This
* name is case-insensitive and punctuation-insensitive.  This is accomplished by
* converting the name to a lowercase and stripping all non alpha-num characters out.
* This simplified version of the name is compared between what the page is linking to
* and the simplified names of the other pages.
*
* Leaves (pages) can have multiple names, so that the name can be changed without
* breaking links -- old reference will continue to work.
*
* Names are not necessarily unique: there can be multiple leaves with the same name,
* because users are not prevented from entering a particular name.  This means that
* there needs to be a "disambiguation" step in the case that there are other pages
* with the same name.
*
* The approach is to have two objects: one object NGPageIndex represents the leaf (page)
* and the other represents a particular name value (NGTerm).  The term object
* represents a many-to-many association between all the pages that link to a term,
* and all the pages that can be targets of that term.  Thus the NGPageIndex object
* has a "name" collection of terms, and a "links" collection of terms.  Each NGTerm
* has a "target pages" collection of index entries, and a "source links" collection
* of index entries.
*
* For example, a page "p1" may have three names: "a", "b", and "c".  The page "p1"
* will be represented by an NGPageIndex object. There will be three NGTerm objects,
* one for each name.  Each of those NGTerm objects will have a single target reference
* back to the page "p1".  If a page "p2" links to "a", then in the link collection
* for "p2" it will have a pointer to the term "a", and term "a" will include "p2"
* in the "source link" collection.
*
* If you are on page "p1", and want to find all the pages that point to "p1", then
* you find all the terms that represent the names of "p1", and find all the pages
* that link to that term.
*
* This has the effect of over counting in the case of non-unique names.  If there
* are three pages that have a single link to "a", and there are three pages that have
* the single name "a", then each of the three target pages will show three pages linking
* to them.  And each page will effectively link to all three of the pages with
* the same name.  Even though the page has only one link.  In practice, the user will
* have to choose the page when traversing the link.
*/
public class NGPageIndex
{
    ...
    */
    ...
    public static Hashtable<String, Vector<ArrayBlockingQueue<String>>> blqList
        = new Hashtable<String, Vector<ArrayBlockingQueue<String>>>();

    public static Hashtable<String, ArrayBlockingQueue<String>> bsnList
        = new Hashtable<String, ArrayBlockingQueue<String>>();
    ...
    private static HashMap<String, List<NGPageIndex>> lockMap
        = new HashMap<String, List<NGPageIndex>>();
    ...
    /**
    * Returns the collection of all target pages that are linked
    * FROM this page.  Vector contains NGPageIndex objects.
    * Remember, this may contain multiple pages for a single
    * link if there are multiple pages with the same name.
    */
    ...
    /**
    * Returns the collection of all source pages that link
    * TO this page.  Vector contains NGPageIndex objects.
    * Remember, this may contain multiple pages for a single
    * link if there are multiple pages with the same name.
    */
    ...
    /**
    * Determine whether there are any inbound links, and if so it is
    * not an orphan, but if there are none, then it is an orphan.
    */
    ...
    */
    public static void refreshOutboundLinks(NGPage aPage)
        throws Exception
    {
    ...
    */
    public static Vector<NGPageIndex> getAllProjectsInAccount(String accountKey) throws Exception
    {
        ...
        for (NGPageIndex ngpi : allContainers)
        {
            ...
            if (ngpi.containerType != CONTAINER_TYPE_PROJECT && ngpi.containerType != CONTAINER_TYPE_PAGE)
            {
    ...
    /**
    * Given a string tag value, looks for and returns
    * all the pages that are tagged with that tag.
    *
    * Must NOT manipulate resulting vector in any way!
    */
    public static Vector<NGPageIndex> getContainersForTag(String tag)
        throws Exception
    {
        if (tag==null)
        {
    ...
    /**
    * Get the container object associated with this index entry,
    * or return a null if one can not be found.
    */
    public NGContainer getContainer()
    throws Exception
    {
        setLock();
    ...
    /**
    * Get the container object associated with this index entry,
    * or throw an exception if that container can not be found.
    * This method never returns a null;
    */
    public NGContainer getContainerOrFail()
    throws Exception
    {
    ...
    */
    public synchronized static void clearAllStaticVars()
    {
        ...
        AccountReqFile.clearAllStaticVars();
    }
    ...
    * to 20 seconds for initialization to complete with either a failure
    * or a successful initialization.
    */
    ...
    /**
    * provides compatibility with earlier version that would attempt to initialize
    * on any given page refresh, and throw an exception if it failed.
    * Newer approach is to initialize once, but this will throw the exception
    * anyway if one was found attempting to initialize.
    */
    ...
    public static void makeIndex(NGContainer ngc)
    throws Exception
    {
    ...
    public static NGPageIndex getContainerIndexByKey(String key)
    throws Exception
    {
        assertInitialized();
        if (key==null)
    ...
    public static NGPageIndex getContainerIndexByKeyOrFail(String key)
    throws Exception
    {
        NGPageIndex ngpi = getContainerIndexByKey(key);
        if (ngpi==null)
    ...
    public static NGContainer getContainerByKey(String key)
        throws Exception
    {
    ...
    public static NGContainer getContainerByKeyOrFail(String key)
        throws Exception
    {
    ...
    public static NGPage getProjectByKeyOrFail(String key)
        throws Exception
    {
    ...
    /**
    * Finding pages by name means that you might find more than one
    * so you get a vector back, which might be empty, it might have one
    * or it might have more pages.
    */
    public static Vector<NGPageIndex> getPageIndexByName(String pageName)
        throws Exception
    {
        assertInitialized();
    ...
    public static Vector<NGPageIndex> getContainerIndexByName(String name)
    throws Exception
    {
        assertInitialized();
    ...
    public static boolean pageExists(String pageName)
        throws Exception
    {
    ...
    public static NGPageIndex[] getAllPageIndex()
        throws Exception
    {
    ...
    */
    ...
    static public String getProjectFullNameList(String matchKey,String bookKey) throws Exception
    {
        ...
        for (NGPageIndex page : getAllProjectsInAccount( bookKey ))
        {
            if(page.containerName.toLowerCase().startsWith(matchKey.toLowerCase()))
            {
                if (addComma)
    ...
    public static void postEventMsg(String emsg){
        ...
        if(v != null){
            //copied out to an array because I had concurrent update problems otherwise.
    ...
    public void setLock()throws Exception{
        ...
        try{
            ...
            String msg =
                ...
                + UPDATE_LOCK_WAIT + "' setting in Config file.";
    ...
    public void clearLock(){
        ...
        try{
            ...
            System.out.println(new Date().toString()  + " " + ctid
                    +  ": clearLock cid: " + this.containerKey
                    + " lock is available nothing to clear");
        }
    }
...
/////////////// INTERNAL PRIVATE METHODS ///////////////////
    ...
    * purely by scanning files, or by adding containers.  The index
    * is built and maintained internally.
    */
    ...
    */
    private void initNameTerms(NGContainer container)
        throws Exception
    {
    ...
    /**
    * Containers have outbound links.  This method finds all the links, and
    * then finds the associated terms,
    * then marks those terms as having source links from this container.
    */
    private void initLinkTerms(NGContainer container)
        throws Exception
    {
        ...
        for (String entry : tmpRef)
        {
            ...
            if (!refTermTmp.contains(term))
            {
                ...
                {
                    throw new NGException("nugen.exception.duplicacy.problem.with.source.leaves",null);
                }
    ...
    /**
    * Page objects can have hash tags in the description.
    * The hash tag starts with a # character and continues to the first white space
    * A NGTerm object is created for each tag, and made to point to this page
    * Each NGPageIndex object contains a vector of such hash terms.
    * If each page lists their hash tags, and links to all the other pages that
    * have that hash tag, then it is an easy way to link similar pages together
    * automatically in both directions.
    */
    private void initIndexForHashTags(NGContainer container)
        throws Exception
    {
    ...
    /**
    * unlinkAll disconnects this NGPageIndex object from the terms
    * so that it can be discarded.  This must be called when an index
    * entry is removed from the index.
    */
