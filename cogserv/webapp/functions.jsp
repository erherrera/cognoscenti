...
%><%!private NGPage ngp = null;
    ...
    public void leafletSectionDisplay(AuthRequest ar, NGSection section,
        ...
        Vector vizComments = SectionComments.getVisibleComments(section, displayLevel,
                ar.getUserProfile());
    ...
    public void displayOldLeaflet(AuthRequest ar, NGSection section,
        ...
        String lastEditor = cr.getLastEditedBy();
        ...
        if (!ar.isLoggedIn() || ar.isStaticSite() || uProf==null) {
            ...
            canEdit = true;
        } else if (cr.getEditable() == NoteRecord.EDIT_MEMBER
                && ngp.primaryOrSecondaryPermission(uProf)) {
            canEdit = true;
        }
        ...
        if (canEdit) {
            ...
            String emailUrl = ar.retPath
                    + "CommentEmail.jsp?p=" + SectionUtil.encodeURLData(ngp.getKey())
            ...
            ar.write(ar.retPath);
            ar.write("emailIcon.gif\"></a> ");
        }

        if (i >= 0 && !ar.isStaticSite()) {
            ar.write("\n<a href=\"");
            ar.write(ar.retPath);
        ...
        if (canEdit) {
             String gwteUrl = ar.retPath + "GWTNoteEditor.jsp?pid="
                                + SectionUtil.encodeURLData(ngp.getKey()) + "&nid="
                                + SectionUtil.encodeURLData(cr.getId());
             ar.write("\n<a href=\"");
             ar.writeHtml(gwteUrl);
             ar.write("\" title=\"GWT Edit Note\"  target=\"_blank\"><img src=\"");
             ar.write(ar.retPath);
             ar.write("gwt-logo.png\" width=\"25\" height=\"25\" /></a>");
        ...
        UserProfile.writeLink(ar, lastEditor);
        ar.write(" ");
        ...
        throws Exception
    {
        ...
        if (leaflet!=null)
        {
            ...
            if (includeBody)
            {
                ...
                WikiConverter.writeWikiAsHtml(ar, leaflet.getData());
                ar.write("\n<hr/>");
            }
            else
            {
    ...
    */
    ...
    //added for container change
    private NGContainer findContainer(String key)
        throws Exception
    {
        NGPageIndex ngpi = NGPageIndex.getContainerIndexByKey(key);
        if (ngpi==null)
        {
            return null;
        }
        return ngpi.getContainer();
    }
